...
const projects = {
  ...
  getProjects(dir = `${__dirname}/projects/`, filter) {
    ...
    if (fs.statSync(dir).isDirectory()) {
      ...
      if (files.length == 0) {
        if (dir.search(filter) < 0) return;
        const split = dir.split('/');
        if (split.length < 5) return;
        const projectName = split[split.length - 2];
        this.projects.push({ name: projectName, files: [], dir });
        return;
      }
      files.forEach((file) => {
        if (fs.statSync(dir + file).isDirectory()) {
          ...
          const split = lastDir.split('/');
          const projectName = split[split.length - 2];
          const src = `../projects/${filter}/${projectName}/${file}`;
          this.files.push(src);
...
io.on('connection', (socket) => {
  ...
  const uploader = new SocketIOFile(socket, {
    ...
    rename(filename) {
      const split = filename.split('.');	// split filename by .(extension)
      const fname = split[0];	// filename without extension
      const ext = split[1];
      let name = '';
      if (uploadProjectName !== '') name = `/${uploadCat}/${uploadProjectName}/${fname}_${count++}.${ext}`;
      else name = `/${uploadCat}/${fname}.${ext}`;
      console.log(name);
      return (name);
    },
  ...
  socket.on('createProject', (project) => {
    const dir = `${__dirname}/projects/${project.category}/${project.name}`;
    ...
    messageModel.find({ $or: [{ owner: id }, { recipient: id }] })
      ...
      .exec((err, messages) => {
        for (let i = 0; i < messages.length; i++) {
          if (messages[i].owner == 777) messages[i].class = 'princess-furniture';
          else messages[i].class = 'user';
        }
        messages.to = id;
      ...
      .exec((err, messages) => {
        for (let i = 0; i < messages.length; i++) {
          ...
          if (dialogs.length !== 0) {
            ...
            dialogs.push({ user, messages: [messages[i]] });
